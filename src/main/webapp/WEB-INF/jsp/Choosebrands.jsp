...
<table class="layui-table" lay-data="{width: 485,    url:' Choosebrandss',  id:'idTests'   ,page: true}" lay-filter="huanzhemlist">
  <thead>
    <tr>
      <!-- <th lay-data="{type:'checkbox', fixed: 'left'}"></th> -->
<!--       <th lay-data="{field:' ',   sort: true, fixed: true}">采购员id</th> -->
       <th lay-data="{field:'prdno',   sort: true, fixed: true}">厂牌名称</th>
      <th lay-data="{field:'producer',   sort: true, fixed: true}">厂牌编码</th>
...
layui.use('table', function(){
  ...
  table.on('tool(huanzhemlist)', function(obj){
    ...
    if(obj.event === 'choices'){
      ...
      parent.$('#cpaibh').val(data.prdno);
      parent.$('#cpai').val(data.producer);
